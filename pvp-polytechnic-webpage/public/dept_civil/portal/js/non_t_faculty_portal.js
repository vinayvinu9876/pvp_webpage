...
var non_t_fac_array;
  let NonTeachingDataRef = firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/');
...
var add_qualification_elem="<h4>QUALIFICATION</h4><input type='text' class='form-control input-sm fac_textbox' id='non_fac_qualification_text"+i+"'><br />";
  ...
  function update_non_faculty_data(location_num)
  {
    ...
    var non_fac_fire_ref=firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/');
  ...
  function upload_new_non_fac()
  {
    ...
    ===========================*/
     ...
     var newPostKey = firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/').child('posts').push().key;

     // Write the new post's data simultaneously in the posts list and the user's post list.
     var updates = {};
     updates['CIVIL/CIVIL_faculty/non_teaching/'+ newPostKey] = facData;
  ...
  function remove_non_fac(location_num)
  {
    ...
    var remove_ref=firebase.storage().ref('CIVIL/fac_images/non_teaching/'+non_t_fac_array[location_num-1][1].img_name);
    remove_ref.delete().then(function(){
      firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]).remove();
  ...
  function update_non_fac_img(location_num)
  {
    ...
    firebase.storage().ref('CIVIL/fac_images/non_teaching/'+non_t_fac_array[location_num-1][1].img_name).delete().then(function(){
  ...
  function upload_non_fac_image_fire(fac_img_file,location_num)
  {
    ...
    var uploadTask=firebase.storage().ref("CIVIL/fac_images/non_teaching"+fac_img_file.name).put(fac_img_file);
      ...
      var  downloadURL = uploadTask.snapshot.downloadURL;
       ...
       firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/img_url').set(downloadURL);
       firebase.database().ref('CIVIL/CIVIL_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/img_name').set(fac_img_file.name);
    });//uploading a list has been completed

...
var fac_array;
  let DataRef = firebase.database().ref('civil/civil_faculty/');
...
get_elem.innerHTML="";
  ...
  for(i=1;i<=num_of_fac;i++)
  {
    ...
    var add_img_elem="<img src='../images1/default_avatar.png' class='img-rounded width='250' heigth='200' id='fac"+i+"_img'><input type='file' id='fac_image_file"+i+"'><input type='button' class='btn btn-primary' value='upload' onclick='update_fac_img("+i+")'> <h3 id='fac"+i+"_name'>Loading..</h3><input type='text' id='fac_name_txt"+i+"' class='form-contro input-sm fac_textbox' onchange='update_faculty_data("+i+")'><hr />";
  var add_design_elem="<h4 id='fac"+i+"_designation'>Loading...</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_designation_text"+i+"' onchange='update_faculty_data("+i+")'><br /><h4 id='fac"+i+"_qualification'></h4><input type='text' class='form-control input-sm fac_textbox' id='fac_qualification_text"+i+"' onchange='update_faculty_data("+i+")'><br /><input type='button' class='btn btn-danger' value='Remove' onclick='remove_fac("+i+")'></div></div> ";
...
function write_all_fac_data(num_of_fac,fac_array)
{
  ...
  for(i=1;i<=num_of_fac;i++)
  {
    var name_elem=document.getElementById("fac"+i+"_name");
    var designation_elem=document.getElementById("fac"+i+"_designation");
    var qualification_elem=document.getElementById("fac"+i+"_qualification");
...
function update_faculty_data(location_num)
{
  ...
  var fac_fire_ref=firebase.database().ref('civil/civil_faculty/'+fac_array[location_num-1][0]+'/');
...
function upload_new_fac()
{
  ...
  UPLOAD TASK
  ===========================*/
      var uploadTask = firebase.storage().ref("civil/fac_images/"+fac_image_file_ref.name).put(fac_image_file_ref);
    ...
    uploadTask.on('state_changed', function(snapshot){
      ...
      if(progress===100)
      {
        ...
        update_fac_sec.innerHTML="<div class='col-lg-4 col-md-4 col-sm-4' data-scroll-reveal='enter from the bottom after 0.4s'><div class='faculty-div form-group'><img src='../images1/default_avatar.png' class='img-rounded' width='250' heigth='200' id='fac_img'><input type='file' id='fac_image_file' onchange='preview_image()'><h3 id='fac_name'>No Name</h3><hr /><input type='text' id='fac_name_txt' class='form-control input-sm fac_textbox' value='No name' onchange='update_textbox()'><h4 id='fac_designation'>Null</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_designation_text' value='null' onchange='update_textbox()'><br /><h4 id='fac_qualification'>Null</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_qualification_text' value='Null' onchange='update_textbox()'><br /><input type='button' class='btn btn-primary' value='Update' onclick='upload_new_fac()'></div></div>";
   ...
   var newPostKey = firebase.database().ref('civil/civil_faculty/').child('posts').push().key;

   // Write the new post's data simultaneously in the posts list and the user's post list.
   var updates = {};
   updates['civil/civil_faculty/'+ newPostKey] = facData;

   console.log("The data has been uploaded");
   new_fac_name.value="No name";
   new_fac_designation.value="Nill";
   new_fac_qualification.value="Nill";
...
function remove_fac(location_num)
{
  ...
  var remove_ref=firebase.storage().ref('civil/fac_images/'+fac_array[location_num-1][1].img_name);
  remove_ref.delete().then(function(){
    firebase.database().ref('civil/civil_faculty/'+fac_array[location_num-1][0]).remove();
...
function update_fac_img(location_num)
{
  ...
  firebase.storage().ref('civil/fac_images/'+fac_array[location_num-1][1].img_name).delete().then(function(){
...
function upload_fac_image_fire(fac_img_file,location_num)
{
  ...
  var uploadTask=firebase.storage().ref("civil/fac_images/"+fac_img_file.name).put(fac_img_file);
    ...
    var  downloadURL = uploadTask.snapshot.downloadURL;
     ...
     firebase.database().ref('civil/civil_faculty/'+fac_array[location_num-1][0]+'/img_url').set(downloadURL);
     firebase.database().ref('civil/civil_faculty/'+fac_array[location_num-1][0]+'/img_name').set(fac_img_file.name);
  });//uploading a list has been completed

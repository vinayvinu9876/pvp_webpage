...
function add_new_post()
{
  ...
  var uploadTask=firebase.storage().ref("SCIENCE/dept_highlights/posts/"+post_image_file.name).put(post_image_file);
    ...
    var newPostKey = firebase.database().ref('SCIENCE/dept_high_lights/posts/').child('posts').push().key;

    // Write the new post's data simultaneously in the posts list and the user's post list.
    var updates = {};
    updates['SCIENCE/dept_high_lights/posts/'+ newPostKey] = postData;
...
function remove_post(location_num)
{
  ...
  var post_ref=firebase.database().ref('SCIENCE/dept_high_lights/posts/'+post_data_array[location_num-1][0]);
  var post_storage_ref=firebase.storage().ref('SCIENCE/dept_highlights/posts/'+post_data_array[location_num-1][1].img_name);
...
function add_notice()
{
  ...
  // Get a key for a new Post.
  var newPostKey = firebase.database().ref('SCIENCE/dept_high_lights/notice/').child('posts').push().key;

  // Write the new post's data simultaneously in the posts list and the user's post list.
  var updates = {};
  updates['/SCIENCE/dept_high_lights/notice/'+ newPostKey] =data_value.value;
  data_value.value="";
...
function add_events()
{
  ...
  // Get a key for a new Post.
  var newPostKey = firebase.database().ref('SCIENCE/dept_high_lights/events/').child('posts').push().key;

  // Write the new post's data simultaneously in the posts list and the user's post list.
  var updates = {};
  updates['/SCIENCE/dept_high_lights/events/'+ newPostKey] =data_value.value;
  data_value.value="";
...
function remove_notice_list(location_num)
{
  ...
  console.log(notice_arr.length-location_num);
    firebase.database().ref('SCIENCE/dept_high_lights/notice/'+notice_arr[notice_arr.length-location_num][0]).remove();
}
function remove_evt_list(location_num)
{
  firebase.database().ref('SCIENCE/dept_high_lights/events/'+events_arr[events_arr.length-location_num][0]).remove();
}

...
var fac_array;
  let DataRef = firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/');
...
get_elem.innerHTML="<center><h2>TEACHING FACULTY</h2></center>";
  var end_row="</div>";
  ...
  for(i=1;i<=num_of_fac;i++)
  {
    ...
    var add_img_elem="<img src='../images1/default_avatar.png' class='img-rounded width='250' heigth='250' id='fac"+i+"_img'><input type='file' id='fac_image_file"+i+"'><input type='button' class='btn btn-primary' value='upload' onclick='update_fac_img("+i+")'> <h3 id='fac"+i+"_name'>Name</h3><input type='text' id='fac_name_txt"+i+"' class='form-contro input-sm fac_textbox' onchange='update_faculty_data("+i+")'><hr />";
    var add_qualification_elem="<h4 id='fac"+i+"fac_qualification'>QUALIFICATION</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_qualification_text"+i+"' onchange='update_faculty_data("+i+")'><br />";
  var add_design_elem="<h4 id='fac"+i+"_designation'>DESIGNATION</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_designation_text"+i+"' onchange='update_faculty_data("+i+")'><br /><h4 id='fac"+i+"_years_of_exp'>Year of experience</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_years_of_exp_text"+i+"' onchange='update_faculty_data("+i+")'><br /><input type='button' class='btn btn-danger' value='Remove' onclick='remove_fac("+i+")'></div></div> ";
...
function write_all_fac_data(num_of_fac,fac_array)
{
  ...
  for(i=1;i<=num_of_fac;i++)
  {
    ...
    var years_of_exp_txt_box=document.getElementById('fac_years_of_exp_text'+i);
    ...
    years_of_exp_txt_box.value=fac_array[i-1][1].years_of_exp;
...
function update_faculty_data(location_num)
{
  ...
  var fac_years_of_exp=document.getElementById('fac_years_of_exp_text'+location_num).value;

  console.log(fac_name,fac_designation,fac_years_of_exp);
  var fac_fire_ref=firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/'+fac_array[location_num-1][0]+'/');
  ...
  fac_fire_ref.child('years_of_exp').set(fac_years_of_exp);
...
function upload_new_fac()
{
  ...
  var new_fac_years_of_exp=document.getElementById('fac_years_of_exp_text');
  try{
  ...
  UPLOAD TASK
  ===========================*/
      var uploadTask = firebase.storage().ref("SCIENCE/fac_images/teaching/"+fac_image_file_ref.name).put(fac_image_file_ref);
    ...
    uploadTask.on('state_changed', function(snapshot){
      ...
      if(progress===100)
      {
        ...
        update_fac_sec.innerHTML="<div class='col-lg-4 col-md-4 col-sm-4' data-scroll-reveal='enter from the bottom after 0.4s'><div class='faculty-div form-group'><h2>TEACHING</h2><img src='../images1/default_avatar.png' class='img-rounded' width='250' heigth='250' id='fac_img'><input type='file' id='fac_image_file' onchange='preview_image()'><h3 id='fac_name'>Name</h3><hr /><input type='text' id='fac_name_txt' class='form-control input-sm fac_textbox' value='name'><h4>Qualification</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_qualification_text' value='qualification'><br /><h4 id='fac_designation'>designation</h4> <input type='text' class='form-control input-sm fac_textbox' id='fac_designation_text' value='designation' ><br /> <h4 id='fac_years_of_exp'>year of exp</h4><input type='text' class='form-control input-sm fac_textbox' id='fac_years_of_exp_text' value='year of exp' ><br /> <input type='button' class='btn btn-primary' value='Update' onclick='upload_new_fac()'></div></div>";
     ...
     years_of_exp : new_fac_years_of_exp.value,
      img_url : downloadURL,
      img_name : fac_image_file_ref.name
   };

   // Get a key for a new Post.
   var newPostKey = firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/').child('posts').push().key;

   // Write the new post's data simultaneously in the posts list and the user's post list.
   var updates = {};
   updates['SCIENCE/SCIENCE_faculty/teaching/'+ newPostKey] = facData;

   console.log("The data has been uploaded");
   new_fac_name.value="name";
   new_fac_qualification.value="qualification";
   new_fac_designation.value="designation";
   new_fac_years_of_exp.value="year of exp";
...
function remove_fac(location_num)
{
  ...
  var remove_ref=firebase.storage().ref('SCIENCE/fac_images/teaching/'+fac_array[location_num-1][1].img_name);
  remove_ref.delete().then(function(){
    firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/'+fac_array[location_num-1][0]).remove();
...
function update_fac_img(location_num)
{
  ...
  firebase.storage().ref('SCIENCE/fac_images/teaching/'+fac_array[location_num-1][1].img_name).delete().then(function(){
...
function upload_fac_image_fire(fac_img_file,location_num)
{
  ...
  var uploadTask=firebase.storage().ref("SCIENCE/fac_images/teaching/"+fac_img_file.name).put(fac_img_file);
    ...
    var  downloadURL = uploadTask.snapshot.downloadURL;
     ...
     firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/'+fac_array[location_num-1][0]+'/img_url').set(downloadURL);
     firebase.database().ref('SCIENCE/SCIENCE_faculty/teaching/'+fac_array[location_num-1][0]+'/img_name').set(fac_img_file.name);
  });//uploading a list has been completed

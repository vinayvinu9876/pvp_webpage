...
var fac_array;
  let DataRef = firebase.database().ref('womens_cell/womens_cell_faculty/');
...
function update_faculty_data(location_num)
{
  ...
  var fac_fire_ref=firebase.database().ref('womens_cell/womens_cell_faculty/'+fac_array[location_num-1][0]+'/');
...
function upload_new_fac()
{
  ...
  UPLOAD TASK
  ===========================*/
      var uploadTask = firebase.storage().ref("womens_cell/fac_images/"+fac_image_file_ref.name).put(fac_image_file_ref);
   ...
   var newPostKey = firebase.database().ref('womens_cell/womens_cell_faculty/').child('posts').push().key;

   // Write the new post's data simultaneously in the posts list and the user's post list.
   var updates = {};
   updates['womens_cell/womens_cell_faculty/'+ newPostKey] = facData;
...
function remove_fac(location_num)
{
  ...
  var remove_ref=firebase.storage().ref('womens_cell/fac_images/'+fac_array[location_num-1][1].img_name);
  remove_ref.delete().then(function(){
    firebase.database().ref('womens_cell/womens_cell_faculty/'+fac_array[location_num-1][0]).remove();
...
function update_fac_img(location_num)
{
  ...
  firebase.storage().ref('womens_cell/fac_images/'+fac_array[location_num-1][1].img_name).delete().then(function(){
...
function upload_fac_image_fire(fac_img_file,location_num)
{
  ...
  var uploadTask=firebase.storage().ref("womens_cell/fac_images/"+fac_img_file.name).put(fac_img_file);
    ...
    var  downloadURL = uploadTask.snapshot.downloadURL;
     ...
     firebase.database().ref('womens_cell/womens_cell_faculty/'+fac_array[location_num-1][0]+'/img_url').set(downloadURL);
     firebase.database().ref('womens_cell/womens_cell_faculty/'+fac_array[location_num-1][0]+'/img_name').set(fac_img_file.name);
  });//uploading a list has been completed

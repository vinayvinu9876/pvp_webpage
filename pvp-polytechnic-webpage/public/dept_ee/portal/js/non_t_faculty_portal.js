...
var non_t_fac_array;
  let NonTeachingDataRef = firebase.database().ref('EE/EE_faculty/non_teaching/');
  ...
  get_elem.innerHTML="<center><h2>TECHNICAL STAFF</h2></center>";
    ...
    for(i=1;i<=num_of_fac;i++)
    {
      ...
      var add_img_elem="<img src='../images1/default_avatar.png' class='img-rounded width='250' heigth='200' id='non_fac"+i+"_img'><input type='file' id='non_fac_image_file"+i+"'><input type='button' class='btn btn-primary' value='upload' onclick='update_non_fac_img("+i+")'> <h3 id='non_fac"+i+"_name'>NAME</h3><input type='text' id='non_fac_name_txt"+i+"' class='form-contro input-sm fac_textbox' onchange='update_non_faculty_data("+i+")'><hr />";
var add_qualification_elem="<h4>QUALIFICATION</h4><input type='text' class='form-control input-sm fac_textbox' id='non_fac_qualification_text"+i+"'><br />";
  ...
  function update_non_faculty_data(location_num)
  {
    ...
    var non_fac_fire_ref=firebase.database().ref('ADFT/ADFT_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/');
  ...
  function upload_new_non_fac()
  {
    ...
    UPLOAD TASK
    ===========================*/
        var uploadTask = firebase.storage().ref("EE/fac_images/non_teaching/"+fac_image_file_ref.name).put(fac_image_file_ref);
     ...
     var newPostKey = firebase.database().ref('EE/EE_faculty/non_teaching/').child('posts').push().key;

     // Write the new post's data simultaneously in the posts list and the user's post list.
     var updates = {};
     updates['EE/EE_faculty/non_teaching/'+ newPostKey] = facData;
  ...
  function remove_non_fac(location_num)
  {
    ...
    var remove_ref=firebase.storage().ref('EE/fac_images/non_teaching/'+non_t_fac_array[location_num-1][1].img_name);
    remove_ref.delete().then(function(){
      firebase.database().ref('EE/EE_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]).remove();
  ...
  function update_non_fac_img(location_num)
  {
    ...
    firebase.storage().ref('EE/fac_images/non_teaching/'+non_t_fac_array[location_num-1][1].img_name).delete().then(function(){
  ...
  function upload_non_fac_image_fire(fac_img_file,location_num)
  {
    ...
    var uploadTask=firebase.storage().ref("EE/fac_images/non_teaching"+fac_img_file.name).put(fac_img_file);
      ...
      var  downloadURL = uploadTask.snapshot.downloadURL;
       ...
       firebase.database().ref('EE/EE_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/img_url').set(downloadURL);
       firebase.database().ref('EE/EE_faculty/non_teaching/'+non_t_fac_array[location_num-1][0]+'/img_name').set(fac_img_file.name);
    });//uploading a list has been completed

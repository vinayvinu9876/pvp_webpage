...
function remove_gal_img(location_num)
{
  $.notify("Removing image");
    ...
    firebase.storage().ref('LIBRARY/gallery/'+rm_img_name).delete().then(function(){
      firebase.database().ref('LIBRARY/gallery/'+gallery_url_arr[no_of_img-location_num][0]).remove();
...
function  add_new_gal_img()
{
  ...
  var uploadTask=firebase.storage().ref("LIBRARY/gallery/"+img_file.name).put(img_file);
    ...
    var newPostKey = firebase.database().ref('LIBRARY/gallery/').child('posts').push().key;

    // Write the new post's data simultaneously in the posts list and the user's post list.
    var updates = {};
    updates['/LIBRARY/gallery/'+ newPostKey] = imageData;
